...
import { ClawAnimation } from "@/components/ClawAnimation";
...
const STEPS: { key: BeazieStage; label: string }[] = [
  { key: "betting", label: "Coin in" },
  { key: "animating", label: "Drop" },
  { key: "revealing", label: "Secret" },
  { key: "settling", label: "Claim" },
];
      ...
      <div className="mx-auto flex w-full max-w-lg flex-col gap-6 px-4 pb-16 pt-28 m500:pt-24">
        <header className="text-center">
          ...
          <p className="mt-3 font-body text-base font-medium leading-snug text-ink/70">
            One drop. One secret prize.
          </p>
        </header>

        <ClawAnimation stage={stage} active={isPlaying || stage === "done"} />
        ...
        {result && (
          ...
          >
            ...
            {basescanPlay && (
              ...
              >
                See the sealed drop <ExternalLink className="h-3.5 w-3.5" />
              </a>
            )}
            <button
              onClick={reset}
        ...
        {!result && (
          <div className="flex flex-col gap-3">
            <p className="text-center font-body text-sm text-ink/55">
              Credit · {PULL_FEE_ETH} ETH
            </p>
            <PlayButton onPlay={play} isPlaying={isPlaying} label="DROP" />
            <p className="text-center font-body text-xs text-ink/45">
              Your prize stays sealed until you claim it.
            </p>
          </div>
        )}
      </div>
